...
- **Disclose progressively.** When the always-read set itself gets big, don't just keep compacting one giant file — split it into a tiny constitution (the few non-negotiables), a flat index of one-line descriptions, and detail files that load only when their task comes up. The library can then grow indefinitely while the always-read cost stays roughly flat, and the right rule shows up at the moment you'd act on it instead of being skimmed once at the top of the session and forgotten. (This is the same idea as tool/skill systems that preload only names and descriptions and load a body on demand.) The same move applies to a single oversized reference doc: give each split file one job and leave a thin index that links them.
